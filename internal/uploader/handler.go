...
// @Description  Uses X-API-Key to identify company and returns a paginated list of folder prefixes under the company slug
// @Tags         uploader
// @Produce      json
// @Param        X-API-Key  header  string  true   "Company API key"
// @Param        prefix     query   string  false  "Folder prefix (relative to company root) e.g. 'invoices/2023'"
// @Param        limit      query   int     false  "Max number of items (default 100)"
// @Param        next_token query   string  false  "Continuation token for pagination"
// @Success      200        {object}  ListFoldersResponse
// @Failure      401        {string}  string "unauthorized"
// @Failure      500        {string}  string "internal error"
// @Router       /uploader/folders [get]
func (h *Handler) ListFolders(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	ci := auth.CompanyIDFromContext(ctx)
	var (
	...
	if ci == "" {
		...
		companyRec, cErr = h.companyRepo.GetByAPIKey(apiKey)
		if cErr != nil {
			http.Error(w, "failed to look up company", http.StatusInternalServerError)
			return
		}
		...
		companyRec, cErr = h.companyRepo.GetByID(ci)
		if cErr != nil {
			http.Error(w, "failed to look up company", http.StatusInternalServerError)
			return
		}
		if companyRec == nil {
			http.Error(w, "invalid CompanyID", http.StatusUnauthorized)
			return
		}
	}
	...
	prefix := q.Get("prefix")        // relative to company root (optional)
	nextToken := q.Get("next_token") // continuation token (optional)

	// Build the full prefix used in S3 (company root + provided prefix)
	fullPrefix := companyRec.CompanySlug + "/"
	if prefix != "" {
		// ensure no leading slash
		p := strings.TrimPrefix(prefix, "/")
		// ensure trailing slash so we list folders under this prefix
		if !strings.HasSuffix(p, "/") {
			p = p + "/"
		}
		fullPrefix = fullPrefix + p
	}

	// Call underlying s3 service to list prefixes (folders). The S3Service implementation
	// is expected to return folder-like prefixes (strings), a continuation token and an error.
	// Signature assumed: ListPrefixes(ctx context.Context, company *company.Company, prefix string, limit int, nextToken string) ([]string, string, error)
	folders, token, err := h.s3Service.ListPrefixes(ctx, companyRec, fullPrefix, limit, nextToken)
	if err != nil {
		http.Error(w, "failed to list folders from storage", http.StatusInternalServerError)
		return
	}

	// Convert returned prefixes to values relative to the requested prefix for clients.
	items := make([]string, 0, len(folders))
	for _, f := range folders {
		rel := f
		if strings.HasPrefix(f, fullPrefix) {
			rel = strings.TrimPrefix(f, fullPrefix)
		}
		// Trim trailing slash for nicer presentation
...
// @Description  Uses X-API-Key to identify company and returns files under a specific folder prefix
// @Tags         uploader
// @Produce      json
// @Param        X-API-Key  header  string  true   "Company API key"
// @Param        folder     query   string  true   "Folder prefix (relative to company root)"
// @Param        limit      query   int     false  "Max number of items (default 50)"
// @Param        offset     query   int     false  "Offset for pagination (default 0)"
// @Success      200        {object}  ListCompanyFilesResponse
// @Failure      400        {string}  string "invalid request"
// @Failure      401        {string}  string "unauthorized"
// @Failure      500        {string}  string "internal error"
// @Router       /uploader/folders/files [get]
func (h *Handler) ListFilesInFolder(w http.ResponseWriter, r *http.Request) {
	...
	if v := q.Get("next_token"); v != "" {
		nextToken = v
	}

	fullPrefix := fmt.Sprintf("%s/%s", companyRec.CompanySlug, strings.TrimPrefix(folder, "/"))

	keys, token, err := h.s3Service.ListFilesInFolder(ctx, companyRec, fullPrefix, limit, nextToken)
	if err != nil {
		http.Error(w, "failed to list files from storage", http.StatusInternalServerError)
		return
	}
